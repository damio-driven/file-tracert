...
### 6. Move cross-volume mantiene UsnFileRef del volume sorgente
...
### 8. Guard di enqueue con tre punti ciechi
...
### 9. Proiezione (§5 CRITICO) + dipendenze non implementate ma UI write shippata
...
### 13. Rivalutazione Blocked: mancano i trigger ('a ogni evento', §4)
...
| C19 | `IndexUpdater.cs:59-61` | Rename non ricalcola `Extension`/`Category` (assegnate solo in `ScanService.cs:389-390`) → filtri ricerca (`FileSearchIndex.cs:217,225`) e `FilterReconciler.cs:62-65` operano su valori morti fino al re-scan | CONFIRMED |
...
| C26 | `QueueService.cs:295` | `SequenceOrder = MaxAsync()+1` letto fuori dalla transazione d'insert, nessun guard di unicità → enqueue concorrenti = ordine duplicato → la feasibility FIFO (che salta solo `e.SequenceOrder > mine`) doppia-conta reciprocamente le riserve | sweep |
...
| P2 | `IndexUpdater.cs:185-186` | `MaterializedPath ==` con collation BINARY (nessuna NOCASE configurata) vs cache in-memory `OrdinalIgnoreCase` (riga 154 — l'incoerenza di collation è certa) → possibile `DirectoryNode` duplicato per case-variant | Integration test: enqueue MoveFile verso 'photos\x' con 'Photos' indicizzata, assert su duplicato |
...
- **WP4 — Guard di enqueue unificato** *(finding 8 + C26 + K5)*: predicato subtree unico case-insensitive, controllo source+target+CreateFolder, SequenceOrder transazionale.
- **WP5 — Correttezza indice/ricerca** *(finding 6, 11, 12 + C19, C16, P2)*: UsnFileRef azzerato, formato date bind, converter UTC globale, Extension/Category al rename, esclusione ereditata, collation MaterializedPath.
